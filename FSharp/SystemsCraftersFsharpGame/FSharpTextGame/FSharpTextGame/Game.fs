...
// Record type - Container for Name Value Pairs
type Details =
    { Name: string
      Description: string }
...
let north exists = exists.North //({ North = northExit}: Exits) = northExit
...
(*
gameEngine.ApplyUpdate(move north)
...
let runParser parser inputChar =
    let (Parser parserFunc) = parser
    parserFunc inputChar
